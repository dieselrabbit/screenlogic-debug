...
from .const import DOMAIN, DEFAULT_SCAN_INTERVAL, LOGGER, MIN_SCAN_INTERVAL
...
async def async_discover_gateways_by_unique_id(hass):
    ...
    try:
        ...
        LOGGER.debug("Discovered hosts: %s", hosts)
    ...
    LOGGER.debug("Discovered gateways: %s", discovered_gateways)
    return discovered_gateways
...
class ScreenLogicDebugFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def async_get_options_flow(config_entry: config_entries.ConfigEntry):
        """Get the options flow for ScreenLogic Debug."""
        return ScreenLogicDebugOptionsFlowHandler(config_entry)
    ...
    async def async_step_dhcp(self, discovery_info: dhcp.DhcpServiceInfo) -> FlowResult:
        ...
        debug_mac = f"{mac}-D"
        await self.async_set_unique_id(debug_mac)
    ...
    async def async_step_gateway_select(self, user_input=None):
        ...
        if user_input is not None:
            ...
            await self.async_set_unique_id(debug_mac, raise_on_progress=False)
            self._abort_if_unique_id_configured()
            return self.async_create_entry(
                title=name_for_mac(debug_mac),
                data={
...
class ScreenLogicDebugOptionsFlowHandler(config_entries.OptionsFlow):
    """Config flow options handler for screenlogic_debug."""
